...
export function simulateCombat({ attacker, defender, attackerEquip, defenderEquip }) {
  ...
  while (round < MAX_ROUNDS) {
    ...
    // ── Lethal-strike short-circuit: a dead defender cannot counter-swing ─────
    // Strike-first means the attacker's killing blow ends the round before the defender
    // acts. Without this, a defender brought to ≤1 HP would still complete its swing and
    // could counter-kill the attacker, then win the double-down check — the "zombie
    // fighter" bug. Only the normal-hit branch can reach here with the defender dead
    // (the dodge/counter branch deals the defender no damage), so defender_action is null.
    if (defenderHP <= 1) {
      rounds.push({ round, attacker_action, defender_action: null, attacker_hp_after: attackerHP, defender_hp_after: defenderHP })
      winner = 'attacker'
      break
    }

    // ── Defender strikes back (skip if a counter already consumed the turn) ───
    // The defender completes its swing so both sides can fall in the same round when the
    // defender was still alive after the attacker's strike; the round-end check then
    // resolves that genuine simultaneous double-KO defensively.
    if (!counterHappened) {
...
/**
 * Writes (or overwrites) today's pw_daily_stats snapshot row.
 * Idempotent — ON CONFLICT (snapshot_date) DO UPDATE means the latest firing wins;
 * with the two titan-cron firings, the 13:00 UTC write is each date's final state.
 * Trailing-24h columns are 13:00-to-13:00 windows, not calendar days.
 */
export async function writeDailySnapshot(sql) {
  await sql`
