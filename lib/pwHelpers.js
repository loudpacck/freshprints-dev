...
/**
 * @deprecated Use simulateCombat instead. Kept for backwards-compatibility
 * until Pass 2 rewrites handlePvPAttack. Do not call for new code.
 *
 * Pure single-roll combat formula (GDD §8.2). No DB writes.
 */
...
export function simulateCombat({ attacker, defender, attackerEquip, defenderEquip }) {
  ...
  if (result === 'win') {
    ...
    // Draw — minor consolation XP for attacker, no glory either side
    xp_earned             = Math.floor(defender.level / 2)
    glory_earned          = 0
    defender_glory_earned = 0
  }
