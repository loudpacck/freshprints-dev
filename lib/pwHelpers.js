 ...
 * This means rank-1 on hard CAN reach legendary (hard table has 5% epic,
 * which bumps to legendary with 100% certainty for rank-1). Intentional —
 * flags as Option A behavior in build summary.
 */
export function rollTitanLootRarity(titanDifficulty, contributionRank) {
  ...
  const finalIdx = Math.min(rolledIdx + bumpRoll, RARITIES.length - 1)
