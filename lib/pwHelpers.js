...
/**
 * Per-member contribution stats for the alliance roster (Phase F). Batched: at most
 * four queries total regardless of member count (stats, townships, temple income,
 * donation aggregates), so a full 25-member alliance resolves in well under 100ms.
 *
 * Returns a Map keyed by user_id → {
 *   combat_power, township_total, temple_income_per_hour,
 *   donation_lifetime_drachma, donation_lifetime_glory, donation_lifetime_items,
 *   power_contribution
 * }. Members absent from a batched result (no townships/temples/donations) are
 * seeded at 0 so the additive power_contribution never collapses to NaN.
 */
export async function computeMemberContributions(sql, allianceId, memberIds) {
  ...
  for (const id of ids) {
    out.set(id, {
      combat_power: 0,
      township_total: 0,
      temple_income_per_hour: 0,
      donation_lifetime_drachma: 0,
      donation_lifetime_glory: 0,
      donation_lifetime_items: 0,
  ...
  const statRows = await sql`
    ...
    FROM pw_player_stats WHERE user_id = ANY(${ids}::uuid[])
  `
  for (const r of statRows) {
    const e = out.get(r.user_id); if (!e) continue
    e.combat_power = Number(r.attack) + Number(r.defense) + Number(r.agility) +
                     Number(r.energy_max) + Number(r.health_max)
  }

  const townRows = await sql`
    SELECT user_id, COALESCE(SUM(level), 0) AS total
    FROM pw_player_townships WHERE user_id = ANY(${ids}::uuid[])
    GROUP BY user_id
  `
  for (const r of townRows) {
    const e = out.get(r.user_id); if (!e) continue
    e.township_total = Number(r.total) || 0
  }

  // Raw temple income rate (no broker/annunaki/township multiplier) — mirrors the
  // value computeAlliancePowerBreakdown feeds into economic power, not the credited rate.
  const templeRows = await sql`
    SELECT pt.user_id,
           COALESCE(SUM(t.income_per_hour * (1 + 0.234 * POWER(pt.upgrade_level::float, 1.03))), 0) AS income
    FROM pw_player_temples pt
    JOIN pw_temples t ON t.type = pt.temple_type
    WHERE pt.user_id = ANY(${ids}::uuid[])
    GROUP BY pt.user_id
  `
  for (const r of templeRows) {
    const e = out.get(r.user_id); if (!e) continue
    e.temple_income_per_hour = Math.round(Number(r.income) || 0)
  }

  const donRows = await sql`
    SELECT donor_user_id, donation_type,
           COALESCE(SUM(amount), 0)      AS amount_total,
           COALESCE(SUM(power_value), 0) AS power_total,
           COUNT(*)                      AS cnt
    FROM pw_alliance_treasury_log
    WHERE alliance_id = ${allianceId} AND donor_user_id = ANY(${ids}::uuid[])
    GROUP BY donor_user_id, donation_type
  `
  for (const r of donRows) {
    const e = out.get(r.donor_user_id); if (!e) continue
    if (r.donation_type === 'drachma')   e.donation_lifetime_drachma = Number(r.amount_total) || 0
    else if (r.donation_type === 'glory') e.donation_lifetime_glory  = Number(r.amount_total) || 0
    else if (r.donation_type === 'item')  e.donation_lifetime_items  = Number(r.cnt) || 0
    // Donated power: items land on the military track, currency on economic — both count.
    e.power_contribution += Number(r.power_total) || 0
  }

  // Roll the live combat + township weight on top of donated power.
